...
Item {
    ...
    Rectangle { width: parent.width; height: 210; gradient: Gradient { GradientStop { position: 0; color: "#0F172A" } GradientStop { position: 1; color: "#1E3A8A" } }
    ...
    Rectangle { width: Math.min(parent.width - 64, 440); height: 480; radius: 22; color: "white"; border.color: "#E5E8F0"; anchors.horizontalCenter: parent.horizontalCenter; anchors.top: parent.top; anchors.topMargin: 150
        Column { anchors.fill: parent; anchors.margins: 28; spacing: 10
            ...
            Rectangle { width: parent.width; height: 46; radius: 12; color: "#1E3A8A"; Text { anchors.centerIn: parent; text: "Create account"; color: "white"; font.bold: true } MouseArea { anchors.fill: parent; onClicked: { if (backend.signup(nameField.text, emailField.text, passwordField.text, confirmPasswordField.text)) stackView.replace(homePage); else authError = backend.getLastAuthError() } } }
        }
    }
    Text { anchors.horizontalCenter: parent.horizontalCenter; anchors.bottom: parent.bottom; anchors.bottomMargin: 20; text: "Already have an account?  Sign in"; color: "#1E3A8A"; font.pixelSize: 12; MouseArea { anchors.fill: parent; onClicked: stackView.pop() } }
}
